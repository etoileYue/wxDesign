...
private:
    ...
    void OnOpen(wxCommandEvent& event) {
        ...
        for (auto& sign : this->signs) {
            AddSymbolMenuItem(sign.getName());
        }
    }
    ...
    void AddSymbolMenuItem(const wxString& symbolName) {
        // 添加符号菜单项
        wxMenuItem* symbolItem = menuSymbol->Append(wxID_ANY, symbolName);
    ...
    void OnSymbolMenuItemSelected(wxCommandEvent& event) {
        ...
        int symbolIndex = event.GetId() - wxID_HIGHEST;

        if (symbolIndex >= 0 && symbolIndex < signs.size()) {
            // 执行符号菜单项对应的操作
            SelectedSigns.push_back(signs[symbolIndex]);
            Refresh();
        }
    }
